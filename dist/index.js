...
import {
  ...
  validate,
  withResolvers
} from "./chunk-TSCNJ2DK.js";
import "./chunk-6ZYDSAP3.js";
import {
  __export
...
async function recoverPublicKey({ hash, signature }) {
  ...
  const { secp256k1: secp256k12 } = await import("./secp256k1-TOXZAHFE.js");
  const signature_ = (() => {
...
function serializeTransactionDeposit(transaction) {
  ...
  const { sourceHash, data, from: from5, gas, isSystemTx, mint: mint2, to, value } = transaction;
  const serializedTransaction = [
    sourceHash,
    from5,
    to ?? "0x",
...
// src/actions/swap.ts
import { ModelType as ModelType2, composePrompt as composePrompt2, elizaLogger as elizaLogger2 } from "@elizaos/core-plugin-v2";
import {
...
var SwapAction = class {
  ...
  async getLifiQuote(fromAddress, params, fromTokenDecimals) {
    try {
      const routes = await getRoutes2({
        ...
        fromAddress,
        options: {
          slippage: params.slippage !== void 0 ? params.slippage / 100 : 5e-3,
          order: "RECOMMENDED"
        }
      });
      if (!routes.routes.length) throw new Error("No routes found");
      ...
      elizaLogger2.error("Error in getLifiQuote:", errorMessage);
      return void 0;
    }
  }
  async getBebopQuote(fromAddress, params, fromTokenDecimals) {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      elizaLogger2.error("Error in getBebopQuote:", errorMessage);
      return void 0;
    }
  }
  async executeLifiQuote(quote) {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      elizaLogger2.error(`Failed to execute lifi quote: ${errorMessage}`);
      return void 0;
    }
  }
  async executeBebopQuote(quote, params) {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      elizaLogger2.error(`Failed to execute bebop quote: ${errorMessage}`);
      return void 0;
    }
  }
};
var buildSwapDetails = async (state, runtime, wp) => {
  ...
  state.chainBalances = Object.entries(balances).map(([chain2, balance]) => {
    const chainConfig5 = wp.getChainConfigs(chain2);
    return `${chain2}: ${balance} ${chainConfig5.nativeCurrency.symbol}`;
  }).join(", ");
...
// src/service.ts
import { Service, elizaLogger as elizaLogger3 } from "@elizaos/core-plugin-v2";
